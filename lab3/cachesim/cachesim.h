...
private:
    ...
    /**
     * @brief H
     * H maps (yn, ... , y1) to (yn XOR y1, yn, ... y2)
     * n = m_sets
     */
    ...
    /**
     * @brief RH
     * The reverser of H, mapping (yn, ... , y1) to (yn-1, ... y1, yn XOR y1)
     * n = m_sets
     */
    unsigned RH(unsigned y);

    /**
     * @brief f
     * Calculate the target set index of `address` at way `i`
     */
    unsigned f(unsigned way_idx, uint32_t address);
