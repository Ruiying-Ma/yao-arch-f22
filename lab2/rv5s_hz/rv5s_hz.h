...
public:
    ...
    SUBCOMPONENT(
        wrong_pc_hazard_or,
        TYPE(Or<1, 2>));  // wrong_pc_hazard_or (Or gate): the result is (syscall || the predicted result is wrong).
    SUBCOMPONENT(
        syscall_hazard_or,
        TYPE(Or<1, 2>));  // syscall_hazard_or (Or gate): the result is (syscall || ID/EX reg is clear due to hazard).
